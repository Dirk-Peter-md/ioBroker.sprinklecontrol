        ...
        try {
            ...
            if (currentPumpUse.enable === true && adapter.config.minimumPressure > pressure) {
                if (sensorPressure.ac === null) {
                    ...
                    const signal = sensorPressure.ac.signal;

                    await asyncTime.setTimeout(60000, undefined, { signal });  // max. 60s warten und Drucksensorwert erneut prüfen, bevor die Pumpe ausgeschaltet wird
                ...
                if (sensorPressure.inadequatePressure === true) {
                    ...
                    adapter.log.debug(`setSensorPressure: Pressure value has been stable for 60 seconds and is now above the minimum pressure. The pump will remain on.`);
                }
                if (sensorPressure.ac !== null && sensorPressure?.ac?.aborted === false) {
            ...
            if (error.name === 'AbortError') {
                adapter.log.debug(`setSensorPressure: Pressure value has been stable for 60 seconds, but is still below the minimum pressure. Turning off the pump.`);
                if (sensorPressure?.ac?.aborted === false) {
                    sensorPressure.ac.abort();   // AbortController zurücksetzen
                    sensorPressure.ac = null;
                }
            }
