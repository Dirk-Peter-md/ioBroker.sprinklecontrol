    ...
    addList (sprinkleList) {
        ...
        for (const res of sprinkleList) {
            ...
            if (!addDone) {
                ...
                adapter.log.warn(`${pipeFlow > adapter.config.triggerMainPumpPower} || (${adapter.config.cisternSettings} && ${pipeFlow > adapter.config.triggerCisternPumpPower}) | ${pipeFlow} > ${adapter.config.triggerCisternPumpPower}`);
                if (adapter.config.triggerCisternPump === true) {
                    if ((pipeFlow > adapter.config.triggerMainPumpPower) || (pipeFlow > adapter.config.triggerCisternPumpPower)) {
                        if (pipeFlow > adapter.config.triggerMainPumpPower) adapter.log.warn(`Emergency irrigation! Valve consumption > pump performance (main pump)`);
                        if (pipeFlow > adapter.config.triggerCisternPumpPower) adapter.log.warn(`Emergency irrigation! Valve consumption > pump performance (cistern pump)`);
                    ...
                    if (pipeFlow > adapter.config.triggerMainPumpPower) {
                        adapter.log.warn(`Emergency irrigation! Valve consumption > pump performance (main pump)`);
