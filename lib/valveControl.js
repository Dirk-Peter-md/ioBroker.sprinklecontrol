    ...
    addList (sprinkleList) {
        ...
        for (const res of sprinkleList) {
            ...
            if (!addDone) {
                ...
                if(res.pipeFlow > adapter.config.triggerMainPumpPower 
                    || (adapter.config.triggerCisternPump 
                        && res.pipeFlow > adapter.config.triggerCisternPumpPower
                    )
                ) {
                    if (res.pipeFlow > adapter.config.triggerMainPumpPower) adapter.log.console.warn(`Emergency irrigation! Valve consumption > pump performance (main pump)`);
                    if (res.pipeFlow > adapter.config.triggerCisternPumpPower) adapter.log.console.warn(`Emergency irrigation! Valve consumption > pump performance (cistern pump)`);
                    res.pipeFlow = (adapter.config.triggerMainPumpPower < adapter.config.triggerCisternPumpPower) ? adapter.config.triggerMainPumpPower : adapter.config.triggerCisternPumpPower;
                }
                ...
                const newThread = {
                    ...
                    pipeFlow: myConfig.config[res.sprinkleID].pipeFlow,  // Wasserverbrauch in l/h   
                    ...
                    litersPerSecond: myConfig.config[res.sprinkleID].pipeFlow / 3600,    // Wasserverbrauchsmenge pro Sekunde   
