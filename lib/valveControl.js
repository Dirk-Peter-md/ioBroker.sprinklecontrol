    ...
    setActive: async(val) => {
        try {
            ...
            if (val === true && sensorPressure.enable) {
                await valveControl.setSensorPressure(sensorPressure.value); // Sensor Druck aktivieren
            } else {
...
const updateList = async () => {
    ...
    const adjustment = (entry) => sumOfWateringTime > entry.wateringTime   
    ...
    if (consumption.pumpRequired === true) {    // Pumpe erforderlich
        ...
        try {
            ...
            || adjustment(controlVoltage) === true  // Anpassung ON_Time wenn Laufzeit zu kurz, damit die 24V Versorgung während der Laufzeit nicht ausgeschaltet wird
            ) {
        ...
        try {
            ...
            || adjustment(currentPumpUse) === true  // Anpassung ON_Time wenn Laufzeit zu kurz, damit die Pumpe während der Laufzeit nicht ausgeschaltet wird
            ) {
            ...
            case 'pumpAndCistern': {
                ...
                /* Bewässerungspumpen aktiv */
                    ...
                    /* (Zisterne unter Minimum) && (ZisternenPumpe läuft) */
                        ...
                        await asyncTime.setTimeout(switchingDistanceMS, undefined, undefined); // Schaltabstand zwischen den Pumpen
                        ...
                        await asyncTime.setTimeout(switchingDistanceMS + currentPumpUse.leadTime, undefined, undefined); // Schaltabstand zwischen den Pumpen
                        if(!sendMessageText.onlySendError()){
...
const valveControl = {
                ...
                if (threadList) {
                    for await (const entry of threadList) {
                        if (entry.sprinkleID === res.sprinkleID) {
                            ...
                            if (entry.autoOn !== true) {    // Automatikbetrieb (true) bevorzugen
    ...
    /**
     * timeBasedRestriction => zeitliche Bewässerungsbeschränkung
     * 
     * @param {boolean} enable
     * @returns {Promise<boolean>} true/false = erfolgreich / nicht erfolgreich
     */
        ...
        try {
            ...
            if (sensorPressure.active === true && adapter.config.minimumPressure > pressure) {
                if (sensorPressure.ac === null) {
                    ...
                    await asyncTime.setTimeout(120000, undefined, { signal: sensorPressure.ac.signal });  // max. 120s warten und Drucksensorwert erneut prüfen, bevor die Pumpe ausgeschaltet wird
            ...
            if (error.name === 'AbortError') {
                ...
                // test Druck o.k. löschen wenn es nichts bringt
                if (sensorPressure.inadequatePressure === true) {
                    alteredPressure = true;
                    sensorPressure.inadequatePressure = false;
                }
                if (sensorPressure?.ac !== null) {
