...
const switchTheValvesOffOn = async (threadList, parallel) => {
    ...
    for (const entry of threadList) {               // ausschalten der Ventile
        if ((!entry.enabled                             //   Ventile ausgeschaltet z.B. Intervall-Beregnung
            ...
            try {
                await adapter.setForeignStateAsync(entry.idState, {
                    val: false,
                    ack: false
                });
    ...
    if (killList.length > 0) {  // erledigte Bewässerungsaufgaben aus der threadList löschen
        delList(killList);
    }  

    /* Pumpe und Steuerspannung ein/aus schalten */
    if (parallel > 0) {
 ...
 */
async function setActualPump () {
    try {
                let switchOnLevelCistern = (parseFloat(adapter.config.triggerMinCisternLevel) + parseFloat(adapter.config.switchOnLevelCistern)) || 5;
        if (switchOnLevelCistern > 100) {
            switchOnLevelCistern = 100;
            adapter.log.info(`Switch-on point limited to 100%`);
        }
        if (switchOnLevelCistern < 5) {
            switchOnLevelCistern = 5;
            adapter.log.info(`Switch-on point increased to 5%`);
        }
        ...
        if (adapter.config.cisternSettings === true) {
            if (currentPumpUse.enable === true) {
                ...
                if ((fillLevelCistern < parseFloat(adapter.config.triggerMinCisternLevel))
                    ...
                    await adapter.setForeignStateAsync(currentPumpUse.pumpName, {
                        val: false,
                        ack: false
                    }); // Pumpe Zisterne Aus
                ...
                if ((fillLevelCistern >= switchOnLevelCistern)
                    ...
                    currentPumpUse.pumpPower = parseInt(adapter.config.triggerMainPumpPower);
                    currentPumpUse.leadTime = parseInt(adapter.config.mainPumpLeadTime)*1000 || 1000;
                    await adapter.setForeignStateAsync('control.restFlow', {
        ...
        adapter.log.error(`setActualPump error: ${error}`);
    }
}   // End setActualPump
            ...
            if (curPumpState) {
                if (pumpOnOff) {
                    if (curPumpState.val === false) {
                        ...
                        await valveDelay(currentPumpUse.leadTime);  /* warten Schaltabstand */
                    }
                } else {
                    if (curPumpState.val !== false) {
                        adapter.setForeignState(currentPumpUse.pumpName, {
