        ...
        if (result) {
            for (const res of result) {
                ...
                if (newEntry.pipeFlow > adapter.config.triggerMainPumpPower) {
                    adapter.log.info(`${newEntry.objectName} cannot irrigate! Consumption (${newEntry.pipeFlow}) higher than pump capacity (${adapter.config.triggerMainPumpPower})`);
                }
                ...
                if (adapter.config.cisternSettings && newEntry.pipeFlow > adapter.config.triggerCisternPumpPower) {
                    adapter.log.info(`${newEntry.objectName} can not irrigate! Consumption (${newEntry.pipeFlow}) higher than cistern pump capacity (${adapter.config.triggerCisternPumpPower})`);
                }
            }
        }
    },
