...
const sendMessageText = {
        ...
        switch (adapter.config.notificationsType) {
            ...
            case 'Telegram':
                ObjMessage = {
                    ...
                    noticeType: (adapter.config.telegramNoticeType === 'longTelegramNotice'),
                    user: adapter.config.telegramUser,
                    onlyError: adapter.config.telegramOnlyError,
                    waiting: (+ adapter.config.telegramWaitToSend * 1000) || 0
                };
                break;
            ...
            case 'Pushover':
                ObjMessage = {
                    ...
                    noticeType: (adapter.config.pushoverNoticeType === 'longPushoverNotice'),
                    deviceID: adapter.config.pushoverDeviceID,
                    onlyError: adapter.config.pushoverOnlyError,
                    waiting: (+ adapter.config.pushoverWaitToSend * 1000) || 0
                };
                break;
            ...
            case 'WhatsApp':
                ObjMessage = {
                    ...
                    noticeType: (adapter.config.whatsappNoticeType === 'longWhatsappNotice'),
                    onlyError: adapter.config.whatsappOnlyError,
                    waiting: (+ adapter.config.whatsappWaitToSend * 1000) || 0
                };
                break;
     ...
     */
    sendMessage (message) {
        // Abbruch, wenn keine Nachricht übergeben wurde
        if (message === null || message === undefined || message === '') {
            adapter.log.debug(`sendMessageText: message is empty`);
            return;
        }
        ...
        adapter.setTimeout(function () {
            switch (ObjMessage.notificationsType) {
                ...
                default:
                    adapter.log.debug(`sendMessageText: no Notification selected`);
                    break;
            }
        }, ObjMessage.waiting);
    },
